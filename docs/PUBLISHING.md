...
8. Update `CLAUDE.md` §0 (the handoff section: version, store status, next
   step) in the same change. `CLAUDE.md` lives in the repo root and is the
   only copy; `../CLAUDE.md` is just a pointer that imports it.
